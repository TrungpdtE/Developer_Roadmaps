...
> Nhóm: Workflow and Messaging  
> Mức ưu tiên: Personal must know

## Học để làm gì?

Điều phối workflow dài, retry, state, timeout và compensation cho nghiệp vụ phức tạp.

## Kiến thức cốt lõi

- Workflow engine quản lý tiến trình dài có retry, timeout, state và compensation.
- Không nhồi workflow dài vào một transaction database.
- Workflow code cần deterministic nếu engine yêu cầu replay.

## Facts cần nhớ

- Mức ưu tiên trong roadmap: **Personal must know**. Bắt buộc học nếu bạn muốn đi Java backend nghiêm túc.
- Nhóm học: **Workflow and Messaging**.
...
```java
public class EnrollmentWorkflowSketch {
    public void enrollStudent(String studentCode, String courseCode) {
        reserveSeat(courseCode);
        try {
            chargeTuition(studentCode, courseCode);
            confirmEnrollment(studentCode, courseCode);
        } catch (RuntimeException exception) {
            releaseSeat(courseCode);
            throw exception;
        }
    }
}
...
- `Workflow Engine Java official documentation`
- `Workflow Engine best practices Java`
- `Workflow Engine production pitfalls`
- `Workflow Engine Spring Boot integration`
