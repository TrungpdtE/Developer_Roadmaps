...
Distributed messaging/streaming system tách compute-storage, multi-tenant.

## Kiến thức cốt lõi

- Message broker giúp service giao tiếp bất đồng bộ và giảm coupling thời gian.
- Consumer phải idempotent vì message có thể bị xử lý lại.
- Cần dead-letter queue, retry policy, schema/versioning và observability.
...
public class EnrollmentEventHandler {
    public void handle(StudentEnrolledEvent event) {
        ...
        sendWelcomeEmail(event.studentCode());
        markProcessed(event.eventId());
    }
}

record StudentEnrolledEvent(String eventId, String studentCode, String courseCode) {
}
```
...
- `Apache Pulsar Java official documentation`
- `Apache Pulsar best practices Java`
- `Apache Pulsar production pitfalls`
- `Apache Pulsar Spring Boot integration`
