...
Gọi API ngoài bằng HTTP client, REST client, GraphQL client, timeout/retry/circuit breaker.

## Kiến thức cốt lõi

- API client production cần timeout, retry có kiểm soát, error mapping và logging.
- REST dễ debug; GraphQL linh hoạt field nhưng cần quản lý query complexity/cache.
- Idempotency quan trọng khi retry request tạo/sửa dữ liệu.

## Facts cần nhớ

- Mức ưu tiên trong roadmap: **Personal must know**. Bắt buộc học nếu bạn muốn đi Java backend nghiêm túc.
- Nhóm học: **API Clients**.
...
- `API Clients Java official documentation`
- `API Clients best practices Java`
- `API Clients production pitfalls`
- `API Clients Spring Boot integration`
