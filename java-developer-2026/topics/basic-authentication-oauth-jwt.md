...
Các cơ chế xác thực/phân quyền phổ biến trong backend API hiện đại.

## Kiến thức cốt lõi

- Authentication trả lời: bạn là ai?
- Authorization trả lời: bạn được làm gì?
- JWT là token có claims, không phải session magic; vẫn cần expiry, signature và key rotation.
...
public class AuthorizationExample {
    public boolean canDeleteCourse(UserPrincipal currentUser, Course course) {
        boolean isAdministrator = currentUser.roles().contains("ADMIN");
        boolean isCourseOwner = course.ownerId().equals(currentUser.userId());
        return isAdministrator || isCourseOwner;
    }
}

record UserPrincipal(String userId, java.util.Set<String> roles) {
}

record Course(String courseId, String ownerId) {
}
```
...
- `Basic Authentication, OAuth, JWT Java official documentation`
- `Basic Authentication, OAuth, JWT best practices Java`
- `Basic Authentication, OAuth, JWT production pitfalls`
- `Basic Authentication, OAuth, JWT Spring Boot integration`
