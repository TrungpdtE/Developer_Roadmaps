...
Bức tranh tổng quan về lưu trữ: relational, NoSQL, search engine, cloud database và use case.

## Kiến thức cốt lõi

- Chọn database theo access pattern, consistency, scale, latency và vận hành.
- Database quan trọng hơn framework vì dữ liệu sống lâu hơn code.
- Backup, migration, monitoring và permission là phần của thiết kế database.
...
```text
Decision checklist:
- Dữ liệu có quan hệ chặt và cần transaction không?
- Query chính là lookup theo key, full-text search, analytics hay time-series?
- Cần scale read, scale write hay global distribution?
- Team có kinh nghiệm vận hành công nghệ đó không?
```
...
- `Databases Java official documentation`
- `Databases best practices Java`
- `Databases production pitfalls`
- `Databases Spring Boot integration`
