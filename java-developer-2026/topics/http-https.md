...
Nền tảng mọi REST API/web backend: request, response, header, status code, TLS và cache.

## Kiến thức cốt lõi

- HTTP request gồm method, URL, headers và body.
- HTTP response gồm status code, headers và body.
- HTTPS là HTTP qua TLS, giúp mã hóa và xác thực endpoint.
...
- `HTTP/HTTPS Java official documentation`
- `HTTP/HTTPS best practices Java`
- `HTTP/HTTPS production pitfalls`
- `HTTP/HTTPS Spring Boot integration`
