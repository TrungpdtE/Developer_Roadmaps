...
Thư viện parse command-line arguments dựa trên annotation.

## Kiến thức cốt lõi

- CLI parser giúp validate argument thay vì tự parse chuỗi thủ công.
- Thư viện tốt hỗ trợ help text, required option, default value và subcommand.
- Tool nội bộ cũng cần UX rõ vì người dùng có thể là chính bạn sau vài tháng.
...
public class ImportStudentsCommand {
    @Parameter(names = "--file", required = true, description = "CSV file path")
    private String filePath;

    public static void main(String[] arguments) {
        ImportStudentsCommand command = new ImportStudentsCommand();
        JCommander.newBuilder().addObject(command).build().parse(arguments);
        System.out.println("Importing students from " + command.filePath);
    }
}
```
...
- `JCommander Java official documentation`
- `JCommander best practices Java`
- `JCommander production pitfalls`
- `JCommander Spring Boot integration`
