...
Các building blocks của ASP.NET Core: hosting, middleware, endpoint, controller, config, auth, caching.

## Kiến thức cốt lõi

- ASP.NET Core request đi qua middleware pipeline rồi tới endpoint.
- Minimal API tốt cho endpoint nhỏ; Controller tốt khi cần convention/filter/model binding phức tạp.
- API production cần validation, exception handling, auth, logging, OpenAPI và tests.
...
app.MapGet("/api/students/{studentCode}", (string studentCode) =>
{
    return Results.Ok(new StudentResponse(studentCode, "Nguyen Van An"));
});
...
- `ASP.NET Core Basics ASP.NET Core`
- `ASP.NET Core Basics .NET official documentation`
- `ASP.NET Core Basics production best practices`
- `ASP.NET Core Basics testing .NET`
