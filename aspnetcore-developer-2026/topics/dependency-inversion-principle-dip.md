...
High-level module phụ thuộc abstraction, không phụ thuộc trực tiếp low-level implementation.

## Kiến thức cốt lõi

- Nguyên tắc thiết kế giúp code ít coupling và dễ test hơn.
- Không áp dụng máy móc; bài toán nhỏ cần giải pháp đơn giản.
- Trong ASP.NET Core, SOLID thường thể hiện qua service nhỏ, interface đúng chỗ, DI và testable design.
...
- `Dependency Inversion Principle (DIP) ASP.NET Core`
- `Dependency Inversion Principle (DIP) .NET official documentation`
- `Dependency Inversion Principle (DIP) production best practices`
- `Dependency Inversion Principle (DIP) testing .NET`
