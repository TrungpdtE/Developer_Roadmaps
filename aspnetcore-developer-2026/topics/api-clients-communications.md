...
> Nhóm: API Clients & Communications  
> Mức: Must know

## Học để làm gì?

Giao tiếp giữa service/client qua REST, gRPC, GraphQL, OData và HTTP clients.

## Kiến thức cốt lõi

- API cần contract rõ: request, response, status code, error format.
- List endpoint phải có paging/filtering/sorting kiểm soát.
- Client communication cần timeout, retry, idempotency và observability.

## Facts cần nhớ

- Mức trong roadmap: **Must know**. Bắt buộc học nếu mục tiêu là đi ASP.NET Core backend nghiêm túc.
- Nhóm học: **API Clients & Communications**.
...
{
    EnrollmentResponse response = await enrollmentService.EnrollAsync(request, cancellationToken);
    return Results.Created($"/api/enrollments/{response.EnrollmentId}", response);
});
```
...
- `API Clients & Communications ASP.NET Core`
- `API Clients & Communications .NET official documentation`
- `API Clients & Communications production best practices`
- `API Clients & Communications testing .NET`
