...
Chạy tác vụ nền bằng hosted service, worker service hoặc scheduler.

## Kiến thức cốt lõi

- Background service chạy ngoài request pipeline.
- Luôn truyền CancellationToken và xử lý shutdown tử tế.
- Không chạy job dài trong request thread nếu có thể đưa vào queue/background worker.
...
```csharp
public sealed class EnrollmentCleanupService : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            Console.WriteLine("Cleaning expired enrollments");
...
- `Background Tasks ASP.NET Core`
- `Background Tasks .NET official documentation`
- `Background Tasks production best practices`
- `Background Tasks testing .NET`
