# Caching Output, Response, Hybrid
...
Các kiểu cache response/output/hybrid để giảm latency và tải backend.

## Kiến thức cốt lõi

- Cache đổi consistency lấy latency/throughput.
- Thiết kế key, TTL, invalidation và cache stampede protection trước khi code.
- Local cache nhanh nhưng mỗi instance có dữ liệu riêng; distributed cache dùng chung nhưng có network cost.
...
```csharp
public sealed class CourseLookupService
{
    private readonly IMemoryCache cache;

    public CourseLookupService(IMemoryCache cache)
    {
        this.cache = cache;
    }

    public Task<string> GetCourseTitleAsync(string courseCode)
    {
        return cache.GetOrCreateAsync($"course-title:{courseCode}", entry =>
        {
            entry.AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(10);
            return Task.FromResult("ASP.NET Core");
        })!;
    }
}
```
...
- `Caching Output, Response, Hybrid ASP.NET Core`
- `Caching Output, Response, Hybrid .NET official documentation`
- `Caching Output, Response, Hybrid production best practices`
- `Caching Output, Response, Hybrid testing .NET`
