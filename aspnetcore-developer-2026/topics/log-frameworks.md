...
Thư viện logging structured, sink, enrichers và log levels.

## Kiến thức cốt lõi

- Structured logging tốt hơn string log vì query được theo property.
- Log cần request id/correlation id để trace request qua service.
- Không log token, password, secret hoặc PII không cần thiết.

## Facts cần nhớ

- Mức trong roadmap: **Must know**. Bắt buộc học nếu mục tiêu là đi ASP.NET Core backend nghiêm túc.
- Nhóm học: **Log Frameworks**.
...
- `Log Frameworks ASP.NET Core`
- `Log Frameworks .NET official documentation`
- `Log Frameworks production best practices`
- `Log Frameworks testing .NET`
