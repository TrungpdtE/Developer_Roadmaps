...
Dùng terminal để tạo project, restore, build, test, publish và quản lý tool/template.

## Kiến thức cốt lõi

- `dotnet new` tạo project, `dotnet restore` tải package, `dotnet build` compile.
- `dotnet test` chạy test, `dotnet publish` tạo output deploy.
- CLI giúp bạn debug CI/CD tốt hơn vì CI cũng chạy command tương tự.
...
```bash
dotnet --info
dotnet new webapi -n StudentEnrollment.Api
dotnet new xunit -n StudentEnrollment.Tests
dotnet sln add StudentEnrollment.Api StudentEnrollment.Tests
dotnet test
```
...
- `Learn dotnet CLI ASP.NET Core`
- `Learn dotnet CLI .NET official documentation`
- `Learn dotnet CLI production best practices`
- `Learn dotnet CLI testing .NET`
