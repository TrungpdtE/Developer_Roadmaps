...
Biết chọn cấu trúc dữ liệu và phân tích độ phức tạp để API không chậm khi dữ liệu tăng.

## Kiến thức cốt lõi

- Biết Big-O để tránh loop lồng nhau trên dữ liệu lớn.
- Dictionary/HashSet giúp lookup nhanh hơn List khi tìm theo key.
- Queue/Stack xuất hiện nhiều trong background processing và thuật toán.
...
public static Dictionary<string, int> CountWords(IEnumerable<string> words)
{
    ...
    {
        counts[word] = counts.TryGetValue(word, out int currentCount)
            ? currentCount + 1
            : 1;
    }
...
- `Data Structures and Algorithms ASP.NET Core`
- `Data Structures and Algorithms .NET official documentation`
- `Data Structures and Algorithms production best practices`
- `Data Structures and Algorithms testing .NET`
