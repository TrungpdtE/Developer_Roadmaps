...
Component model dùng trong Blazor/Razor, tái sử dụng UI C# + markup.

## Kiến thức cốt lõi

- Razor trộn HTML markup với C# expression để render UI/template.
- Razor Pages phù hợp CRUD server-rendered đơn giản.
- Razor Components là nền tảng component UI cho Blazor.
...
{
    <p>@student.FullName (@student.StudentCode)</p>
}

@code {
    private List<StudentViewModel> Students { get; } = new();
}
```
...
- `Razor Components ASP.NET Core`
- `Razor Components .NET official documentation`
- `Razor Components production best practices`
- `Razor Components testing .NET`
