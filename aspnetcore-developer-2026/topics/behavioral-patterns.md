...
Strategy, Command, Observer, Template Method, Mediator: nhóm pattern hành vi.

## Kiến thức cốt lõi

- Pattern là ngôn ngữ chung để mô tả giải pháp.
- Không áp pattern chỉ để code trông phức tạp.
- Trong .NET, Strategy, Decorator, Mediator, Options, Repository thường gặp.

## Facts cần nhớ

- Mức trong roadmap: **Must know**. Bắt buộc học nếu mục tiêu là đi ASP.NET Core backend nghiêm túc.
- Nhóm học: **Design Patterns**.
...
- `Behavioral Patterns ASP.NET Core`
- `Behavioral Patterns .NET official documentation`
- `Behavioral Patterns production best practices`
- `Behavioral Patterns testing .NET`
