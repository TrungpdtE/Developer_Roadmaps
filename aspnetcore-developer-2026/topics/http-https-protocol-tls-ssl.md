...
Nền tảng web API: method, header, status code, JSON, cache, TLS và bảo mật truyền tải.

## Kiến thức cốt lõi

- Request gồm method, URL, headers, body; response gồm status, headers, body.
- Dùng status code đúng: 200, 201, 204, 400, 401, 403, 404, 409, 422, 500.
- HTTPS/TLS là bắt buộc cho API chứa dữ liệu user/token.
...
- `HTTP/HTTPS protocol + TLS/SSL ASP.NET Core`
- `HTTP/HTTPS protocol + TLS/SSL .NET official documentation`
- `HTTP/HTTPS protocol + TLS/SSL production best practices`
- `HTTP/HTTPS protocol + TLS/SSL testing .NET`
