...
Ba chiến lược load navigation property, ảnh hưởng performance và N+1 query.

## Kiến thức cốt lõi

- DbContext là unit of work và identity map trong EF Core.
- LINQ query được dịch sang SQL; không phải mọi C# expression đều dịch được.
- Migrations phải được version-control và review như code.
...
StudentEntity? student = await dbContext.Students
    .SingleOrDefaultAsync(student => student.StudentCode == "S001", cancellationToken);
```
...
- `Lazy Loading, Eager Loading, Explicit Loading ASP.NET Core`
- `Lazy Loading, Eager Loading, Explicit Loading .NET official documentation`
- `Lazy Loading, Eager Loading, Explicit Loading production best practices`
- `Lazy Loading, Eager Loading, Explicit Loading testing .NET`
