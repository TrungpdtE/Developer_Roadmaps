...
Quan sát hệ thống production bằng metrics, logs, traces và alerts.

## Kiến thức cốt lõi

- Metrics trả lời hệ thống khỏe không; logs trả lời chuyện gì đã xảy ra; traces trả lời request đi qua đâu.
- Alert nên gắn với user impact/SLO, không chỉ CPU cao.
- OpenTelemetry giúp instrumentation vendor-neutral.

## Facts cần nhớ

- Mức trong roadmap: **Must know**. Bắt buộc học nếu mục tiêu là đi ASP.NET Core backend nghiêm túc.
- Nhóm học: **Observability**.
...
- `Monitoring/Logging/Tracing/Alerting ASP.NET Core`
- `Monitoring/Logging/Tracing/Alerting .NET official documentation`
- `Monitoring/Logging/Tracing/Alerting production best practices`
- `Monitoring/Logging/Tracing/Alerting testing .NET`
