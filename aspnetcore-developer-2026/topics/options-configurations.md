...
Configuration từ appsettings, environment, secrets; Options pattern để bind config type-safe.

## Kiến thức cốt lõi

- Configuration đến từ appsettings, environment variables, user secrets, command line và secret store.
- Options pattern giúp bind config vào class có validation.
...
{
    public int MaximumCoursesPerStudent { get; set; } = 5;
}
```
...
- `Options & Configurations ASP.NET Core`
- `Options & Configurations .NET official documentation`
- `Options & Configurations production best practices`
- `Options & Configurations testing .NET`
